...
function swedishHolidaySet(year) {
  const set = new Set();
  set.add(new Date(year, 0, 1).toDateString());
  set.add(new Date(year, 0, 6).toDateString());
  set.add(new Date(year, 4, 1).toDateString());
  set.add(new Date(year, 5, 6).toDateString());
  set.add(new Date(year, 11, 25).toDateString());
  set.add(new Date(year, 11, 26).toDateString());
  ...
  set.add(goodFriday.toDateString());
  set.add(easterMon.toDateString());
  set.add(ascension.toDateString());
...
function isWorkday(date) {
  ...
  const holidays = swedishHolidaySet(y);
  return !isWeekend(date) && !holidays.has(new Date(y, date.getMonth(), date.getDate()).toDateString());
}
...
export default function App() {
  ...
  const nowRef = useRef(toZonedDate());
  const [, forceRender] = useState(0);

  useEffect(() => {
    const id = setInterval(() => {
      nowRef.current = toZonedDate();
      forceRender((x) => x + 1);
    }, 100);
  ...
  const {
    ...
    const workingDays = countWorkingDaysInYear(year);
    const workSecondsYear = workingDays * 8 * 3600;
    ...
    <div className="min-h-screen w-full bg-slate-50 text-slate-900 flex flex-col items-center p-4">
      <div className="w-full max-w-md">
        <header className="mt-2 mb-4">
          ...
          <p className="text-slate-600 text-sm">Beräknar intjänande per sekund före/efter skatt under arbetstid (vardagar 09–17) och pausar utanför.</p>
        </header>
        ...
        <section className="bg-white rounded-2xl shadow p-4 space-y-3">
          ...
          <div>
            ...
            />
            <p className="text-xs text-slate-500 mt-1">Tips: slå upp din kommunalskatt och ev. kyrkoavgift och fyll i nedan.</p>
          </div>
            ...
            <div>
              ...
              <div className="mt-2 flex items-center gap-2">
                <input
                  ...
                  type="checkbox"
                  checked={kyrkan}
                  onChange={(e) => setKyrkan(e.target.checked)}
                />
